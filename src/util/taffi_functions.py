...
from src.util.properties import el_radii, el_max_bonds, el_mass
from src.util.hashes import atom_hash


########################################################################################################################
# Begin adjacency matrix generation and manipulation functions
########################################################################################################################
...
def table_generator(elements, geometry, scale_factor=1.2, filename=None):
    ...
    for i in elements:
        if i not in el_radii.keys():
            print("ERROR in Table_generator: The geometry contains an element ({}) "
                  "that the Table_generator function doesn't have bonding information for. "
                  "This needs to be directly added to the Radii".format(i) +
...
def adjmat_to_adjlist(adj_mat):
    """
    Convert between adjacency matrix and adjacency list (actually a list of sets for convenience)
    """
...
########################################################################################################################
# End adjacency matrix generation and manipulation functions
########################################################################################################################

########################################################################################################################
# Begin atom classification functions
########################################################################################################################
...
def ring_path(adj_list, ring, path=None):
    """
    Convenience function for generating an ordered sequence of indices that enumerate a ring starting
    from the set of ring indices generated by return_ring_atoms()
    """
    ...
    # This for recursive construction is needed to handle branching possibilities.
    # All branches are followed and only the one yielding the full cycle is returned
...
def ring_atom(adj_mat, idx, start=None, ring_size=10, counter=0, avoid_set=None, in_ring=None):
    """
    Function to determine whether given atom index in the input sturcture locates on a ring or not.
    ...
    Returns
    -------
        Bool value depending on if idx is a ring atom 
    ...
    status = 0
    cons = [count_i for count_i, i in enumerate(
        adj_mat[idx]) if i == 1 and count_i not in avoid_set]
...
def is_nitro(i, adj_mat, elements):
    ...
    if elements[i] not in ["N", "n"]:
        return False
    O_ind = [count_j for count_j, j in enumerate(adj_mat[i]) if j == 1 and elements[count_j] in [
        "o", "O"] and sum(adj_mat[count_j]) == 1]
    if len(O_ind) == 2:
        return True
    else:
        return False
...
def is_sulfoxide(i, adj_mat, elements):
    ...
    O_ind = [count_j for count_j, j in enumerate(adj_mat[i]) if j == 1 and elements[count_j] in [
        "o", "O"] and sum(adj_mat[count_j]) == 1]
    C_ind = [count_j for count_j, j in enumerate(
        adj_mat[i]) if j == 1 and elements[count_j] in ["c", "C"]]
...
def is_phosphate(i, adj_mat, elements):
    ...
    if elements[i] not in ["P", "p"]:
        return False
    O_ind = [count_j for count_j, j in enumerate(
        adj_mat[i]) if j == 1 and elements[count_j] in ["o", "O"]]
...
def is_cyano(i, adj_mat, elements):
    ...
    if elements[i] not in ["N", "n"] or sum(adj_mat[i]) > 1:
        return False
    C_ind = [count_j for count_j, j in enumerate(adj_mat[i]) if j == 1 and elements[count_j] in [
        "c", "C"] and sum(adj_mat[count_j]) == 2]
    if len(C_ind) == 1:
        return True
    else:
        return False
...
def is_frag_ethenone(i, adj_mat, elements):
    ...
    OS_ind = [count_j for count_j, j in enumerate(adj_mat[i]) if j == 1 and elements[count_j] in [
        "o", "O", "s", "S"] and sum(adj_mat[count_j]) == 1]
    CN_ind = [count_j for count_j, j in enumerate(
        adj_mat[i]) if j == 1 and elements[count_j] in ["c", "C", "n", "N"]]
    connect = sum(adj_mat[i])
...
########################################################################################################################
# End atom classification functions
########################################################################################################################


########################################################################################################################
# Begin atom hash cannonicalization functions
########################################################################################################################
...
########################################################################################################################
# End atom hash cannonicalization functions
########################################################################################################################
