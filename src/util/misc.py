...
from src.util.taffi_functions import gen_subgraphs
...
def gen_labels_for_smiles(y, atom_mapping=False):
    ...
    for count_i, i in enumerate(y.elements):
        ...
        if i == "h" and sum(y.adj_mat[count_i]) == 1 and y.elements[np.where(y.adj_mat[count_i] == 1)[0][0]] != "h":
            ...
            explicit = True
        labels += [(i.title(), explicit, int(len([_ for count, _ in enumerate(y.adj_mat[count_i])
                    if (_ == 1 and y.elements[count] == "h" and sum(y.adj_mat[count]) == 1)])), None, None)]
    return labels
...
def Dijkstra(adj_mat, start=0, end=-1):
    ...
    # Holds shortest distance to origin from each site
    distances = np.array([100000]*(len(adj_mat)))
    # Sets the separation of the initial node from the initial node to zero
    distances[start] = 0
    # Holds the previous site on the short distance to origin
    previous = np.array([-1]*len(adj_mat))
    ...
    while (0 in visited):
        ...
        tmp = min([j for count_j, j in enumerate(
            distances) if visited[count_j] == 0])
        i = [count_j for count_j, j in enumerate(
            distances) if j == tmp and visited[count_j] == 0][0]
