...
from yarp.util.config import InitalStructConfig, JobManagerConfig, EnumerationConfig, PreEnumFilters, PropertyFilterConfig, ProductBlindersConfig, PostEnumFilters, MLPropConfig, ConformerConfig, RPOptConfig, TSOptConfig, TSGuessConfig, IRCValConfig, InitialGeomConfig, GeomSourceConfig, PreOptimizationConfig, ProductPreOptimizationConfig, JointOptimizationConfig
...
class InputParser:
    ...
    def __init__(self, file_dict: dict):
        ...
        self.stage_filters = {}
        # Set while parsing ``init_rxn_path``.  Enumeration consults this to
        # avoid an otherwise redundant product-only UFF optimization when the
        # explicit xTB -> UFF-joint -> xTB preparation workflow is selected.
        self.uses_preoptimized_ts_geometries = False
    ...
    def _parse_stage(self, name: str, data: dict) -> StageConfig:
        ...
        if method == "ml_rxn_prop":
            ...
            # Direct ``ts_guess.geometry_source: preoptimized`` is retained
            # as an explicit one-pair GSM mode.  The normal new workflow uses
            # ``conf_gen.geometry_source: preoptimized`` instead: it prepares
            # xTB R/P minima first, *then* runs CREST from those minima.
            use_preoptimized = tsg_cfg.geometry_source == 'preoptimized'
            conf_data = data.get('conf_gen', {})
            if not use_preoptimized and not conf_data:
                raise ValueError(
                    f"Stage '{name}' needs a conf_gen block unless "
                    "ts_guess.geometry_source is 'preoptimized'."
                )
            conf_cfg = None if use_preoptimized else ConformerConfig(**{
                k: v for k, v in conf_data.items()
                if k in ConformerConfig.__dataclass_fields__
            })
            conf_uses_preoptimized = (
                conf_cfg is not None and conf_cfg.geometry_source == 'preoptimized'
            )
            self.uses_preoptimized_ts_geometries |= (
                use_preoptimized or conf_uses_preoptimized
            )
            ...
            if not use_preoptimized:
                config.tasks[r_conf_id] = TaskDef(
                    ...
                    config=conf_cfg,
                    requires_data=(
                        ["prepared_reactant_geometry"] if conf_uses_preoptimized else []
                    ),
                ...
                config.tasks[p_conf_id] = TaskDef(
                    ...
                    config=conf_cfg,
                    requires_data=(
                        ["prepared_product_geometry"] if conf_uses_preoptimized else []
                    ),
            ...
            config.tasks[tsg_id] = TaskDef(
                ...
                task_type="ts_guess",
                parent_stage=name,
                depends_on=[] if use_preoptimized else [r_conf_id, p_conf_id],
                config=tsg_cfg,
                requires_data=(
                    ["prepared_reactant_geometry", "prepared_product_geometry"]
                    if use_preoptimized else ["reactant_conf", "product_conf"]
                ),
                provides_data=["ts_guess"]
            )

        elif method == 'prepare_initial_geometries':
            reactant_data = data.get('reactant_preopt')
            product_data = data.get('product_preopt')
            joint_data = data.get('joint_opt', {})
            if not reactant_data or not product_data:
                raise ValueError(
                    f"Stage '{name}' requires both 'reactant_preopt' and 'product_preopt' blocks."
                )

            reactant_cfg = PreOptimizationConfig(**{
                key: value for key, value in reactant_data.items()
                if key in PreOptimizationConfig.__dataclass_fields__
            })
            joint_cfg = JointOptimizationConfig(**{
                key: value for key, value in joint_data.items()
                if key in JointOptimizationConfig.__dataclass_fields__
            })
            product_kwargs = {
                key: value for key, value in product_data.items()
                if key in ProductPreOptimizationConfig.__dataclass_fields__
            }
            product_kwargs['joint_opt'] = joint_cfg
            product_cfg = ProductPreOptimizationConfig(**product_kwargs)

            reactant_id = f"{name}.reactant_optimization"
            product_id = f"{name}.product_optimization"
            config.tasks[reactant_id] = TaskDef(
                task_id=reactant_id,
                task_type="reactant_optimization",
                parent_stage=name,
                depends_on=[self.ml_task_id] if self.ml_task_id else [],
                config=reactant_cfg,
                provides_data=["prepared_reactant_geometry"],
            )
            config.tasks[product_id] = TaskDef(
                task_id=product_id,
                task_type="product_optimization",
                parent_stage=name,
                depends_on=[],
                config=product_cfg,
                requires_data=["prepared_reactant_geometry"],
                provides_data=["prepared_product_geometry"],
            )
