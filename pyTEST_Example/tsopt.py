...
class TSOPT:
    ...
    def Initialize(self, verbose = False):
        ...
        if verbose: print(f"rxn_index: {self.rxn_ind}\n", flush = True)

        if os.path.isdir(self.wf) is False: os.mkdir(self.wf)
        self.inp_xyz=f"{self.wf}/{self.rxn_ind}.xyz"
        if args["constrained_TS"] is True:
            xyz_write(self.inp_xyz, self.rxn.reactant.elements, self.rxn.constrained_TS[ind])
        elif args["skip_low_TS"] is True:
            xyz_write(self.inp_xyz, self.rxn.reactant.elements, self.rxn.TS_guess[ind])
        elif args["skip_low_IRC"] is True:
            xyz_write(self.inp_xyz, self.rxn.reactant.elements, self.rxn.TS_xtb[ind])
        else:
            xyz_write(self.inp_xyz, self.rxn.reactant.elements, self.rxn.TS_xtb[ind])
    ...
    def Read_Result(self):
        ...
        if self.dft_job.calculation_terminated_normally() and self.dft_job.optimization_converged() and self.dft_job.is_TS():
            ...
            if self.dft_lot not in self.rxn.TS_dft.keys(): self.rxn.TS_dft[self.dft_lot]=dict()
            conf_i = self.index
            ...
            print(f"rxn: {self.rxn_ind}, ts_dft SPE: {self.rxn.TS_dft[self.dft_lot][conf_i]['SPE']}\n")
