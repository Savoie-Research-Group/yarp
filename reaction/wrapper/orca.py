...
class ORCA:
    def __init__(self, input_geo, work_folder=os.getcwd(), lot='B97-3c', jobtype='ENGRAD', nproc=1, mem=4000, scf_iters=300, jobname='orcajob', charge=0, multiplicity=1,\
        ...
        self.dielectic    = float(dielectric)
        if solvent=="read":
            self.sovation = f"{solvation_model}"
        elif solvent:
    ...
    def generate_irc_settings(self, max_iter=60, print_level=1, oldhess=False):
        ...
        else: info += f'  inhess calc_anfreq\n'
        info += 'end\n\n'
        self.irc = info
