...
from egat.molecule import Molecule, OLD_BOND_ENCODE
from egat.graphgenhelperfunctions import return_reactive
from egat.RDKitHelpers import getInchifromSMILES, RemoveMapping
...
if __name__ == '__main__':
    import os
    _root = Path(__file__).resolve().parent.parent.parent  # EGAT_container
    config_file = _root / "models" / "Activation_barrier.yaml"
    dataset_path = _root / "examples" / "sample_reactions.csv"
    if not config_file.exists() or not dataset_path.exists():
        print("Run from EGAT_container with models/ and examples/ present.")
        raise SystemExit(1)
    config = omegaconf.OmegaConf.load(config_file)
    omegaconf.OmegaConf.set_struct(config, False)
    config.smiles = "reaction_smiles"
    config.data_path = str(_root / "tmp_data")
    os.makedirs(config.data_path, exist_ok=True)
    df = pd.read_csv(dataset_path)
    print(df.iloc[0, :])
    dataset = FastDataset(config, dataset=str(dataset_path))
    print(dataset[0])