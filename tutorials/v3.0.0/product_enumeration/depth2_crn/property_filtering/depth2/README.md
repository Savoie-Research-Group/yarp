...
1. Run `yarp-init input.yaml`
    ...
    - You should see `3HP_depth2.pkl` and `STATUS.json` files generated in the same directory level as `input.yaml`
2. Run `yarp-progress .` from the same directory level as `3HP_depth2.pkl` and `STATUS.json`
